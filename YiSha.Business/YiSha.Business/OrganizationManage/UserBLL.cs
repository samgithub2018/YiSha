...
namespace YiSha.Business.OrganizationManage
{
    public class UserBLL
    {
        ...
        public async Task<TData<List<ZtreeInfo>>> GetZtreeByRole()
        {
            var userList = await userService.GetZtreeInfo();
            TData<List<ZtreeInfo>> rList = new TData<List<ZtreeInfo>>() { Result = userList.ToList() };
            rList.Tag = 1;
            return rList;
        }
        ...
        public async Task<TData<string>> SaveForm(UserEntity entity)
        {
            ...
            entity.DepartmentstoreId = Operator.Instance.Current().Result.DepartmentstoreId;
