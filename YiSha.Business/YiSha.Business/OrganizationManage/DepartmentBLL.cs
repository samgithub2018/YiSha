...
namespace YiSha.Business.OrganizationManage
{
    public class DepartmentBLL
    {
        ...
        public async Task<TData<List<ZtreeInfo>>> GetDepmentForShopJson()
        {
            var List = await departmentTypeService.GetList(new DepartmentTypeListParam() { TypeCode = "2" });
            List<DepartmentEntity> departmentList = await departmentService.GetList(new DepartmentListParam() { DepartmentTypeId = List[0].Id });
            TData<List<ZtreeInfo>> list = new TData<List<ZtreeInfo>>();
            list.Result = new List<ZtreeInfo>();
            departmentList.ForEach(d =>
            {
                list.Result.Add(new ZtreeInfo()
                {
                    id = d.Id,
        ...
        public async Task<TData<string>> SaveForm(DepartmentEntity entity)
        {
            ...
            else if (entity.DepartmenttypeId == null)
            {
                obj.Message = "部门类型不能为空！";
                return obj;
            }
